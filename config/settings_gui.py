...
class HyprConfGUI(Window):
    ...
    def create_system_tab(self):
        ...
        self.auto_append_switch = Gtk.Switch(
            ...
            tooltip_text="Automatically append YZ-Shell source string to hyprland.conf",
        )
        ...
        monitor_label = Label(
            label="Show YZ-Shell on monitors:", h_align="start", v_align="center"
        )
    ...
    def _add_ical_source_widget(self, source):
        """Helper to add an iCal source entry widget to the ical_entries box."""
        bar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        
        # Name entry
        name_entry = Entry(text=source.get('name', ''), h_expand=False, style="min-width: 150px;")
        bar.add(name_entry)
        
        # URL entry
        url_entry = Entry(text=source.get('url', ''), h_expand=True)
        bar.add(url_entry)
        
        # Color button using Gtk.ColorButton
        color_btn = Gtk.ColorButton()
        color_btn.set_use_alpha(False)
        # Parse hex color and set it
        color_hex = source.get('color', '#007acc')
        ...
        try:
            ...
            color = Gdk.RGBA(red=0.0, green=0.48, blue=0.8, alpha=1.0)
            color_btn.set_rgba(color)
        ...
        x_btn.connect(
            "clicked",
            lambda _, current_bar_to_remove=bar: self.ical_entries.remove(
                current_bar_to_remove
            ),
        )
        bar.add(x_btn)
        ...
        bar.name_entry = name_entry
        bar.url_entry = url_entry
        bar.color_btn = color_btn
        
        self.ical_entries.add(bar)
        self.ical_entries.show_all()

    def _on_add_ical_source_clicked(self, button):
        """Handle adding a new iCal source."""
        new_source = {'name': 'New Calendar', 'url': '', 'color': '#007acc'}
        self._add_ical_source_widget(new_source)
    ...
    def create_about_tab(self):
        ...
        settings_box = Box(orientation="h", spacing=10, h_align="start", style="margin-top: 20px;")
        settings_box.add(Label(label="Settings Window:", h_align="start"))
        self.resizable_switch = Gtk.Switch()
        self.resizable_switch.set_active(bind_vars.get('settings_window_resizable', False))
        settings_box.add(self.resizable_switch)
        settings_box.add(Label(label="Resizable", h_align="start"))
    ...
    def on_accept(self, widget):
        ...
        current_bind_vars_snapshot["calendar_start_monday"] = (
            self.calendar_start_monday_switch.get_active()
        )
    ...
    def on_reset(self, widget):
        ...
        if dialog.run() == Gtk.ResponseType.YES:
            ...
            self.terminal_entry.set_text(get_bind_var("terminal_command"))
            self.calendar_start_monday_switch.set_active(
                get_bind_var("calendar_start_monday")
            )
            self.auto_append_switch.set_active(get_bind_var("auto_append_hyprland"))
            self.caffeine_switch.set_active(get_bind_var("caffeine_on_start"))
